...
def build_dataprep_config_compat(config_cls, **kwargs):
    """Создает DataPrepConfig только с поддерживаемыми текущим рантаймом полями."""
    supported_params = inspect.signature(config_cls).parameters
    filtered_kwargs = {key: value for key, value in kwargs.items() if key in supported_params}
    dropped_params = sorted(set(kwargs) - set(filtered_kwargs))
    return config_cls(**filtered_kwargs), dropped_params
...
def build_mlx_training_advice(
    ...
    """Строит рекомендации для MLX fine-tuning по текущим параметрам и данным."""
    source = Path(source_path) if source_path else None
    if not source or not source.exists() or not source.is_file():
        return None

    stats = analyze_dataset(source)
    # Для UI-оценки MLX берём универсальную грубую оценку (1 токен ~= 2.2-3 символа).
    estimated_tokens = max(1, int(max(stats.get('total_words', 0) * 1.2, stats.get('total_chars', 0) / 2.6)))

    train_jsonl = Path(jsonl_dir) / "train.jsonl" if jsonl_dir else None
    train_count = 0
    over_limit = 0
    p95_tokens = 0
    max_tokens_seen = 0

    if train_jsonl and train_jsonl.exists():
        lengths: list[int] = []
        with open(train_jsonl, encoding="utf-8") as f:
            for line in f:
                ...
                text = rec.get("text")
                if text is None and "messages" in rec:
        ...
        if lengths:
            lengths.sort()
            train_count = len(lengths)
            over_limit = sum(1 for t in lengths if t > max_seq_length)
    ...
    # Memory pressure proxy: roughly scales with batch_size * seq_len.
    mem_pressure = batch_size * max_seq_length
    if mem_pressure >= 4096:
        severity = "warning"
        messages.append("Нагрузка на память высокая для Apple Silicon (batch_size × max_seq_length).")
        recommendations.append(
            f"Снизьте batch_size до {max(1, batch_size // 2)} или max_seq_length до {max(512, max_seq_length - 256)}."
        )

    if not grad_checkpoint and mem_pressure >= 3072:
        severity = "warning"
        messages.append("Gradient checkpointing выключен при заметной нагрузке на память.")
        recommendations.append("Включите Gradient checkpointing для уменьшения риска OOM.")

    if val_batches * max_seq_length >= 8192:
        severity = "warning"
        messages.append("Валидация может быть тяжёлой по памяти/времени.")
        recommendations.append(f"Уменьшите Val batches до {max(5, min(10, val_batches // 2))}.")

    if train_count > 0:
        over_ratio = over_limit / train_count
        if over_ratio > 0.1:
            severity = "warning"
            messages.append(
                f"{over_limit} из {train_count} train-примеров ({over_ratio*100:.1f}%) длиннее max_seq_length и будут обрезаться."
            )
            prep_target = max(512, int(max_seq_length * 0.88) // 64 * 64)
            recommendations.append(
                f"При подготовке JSONL ставьте Max sequence length около {prep_target} (чуть ниже train-лимита)."
            )
        elif over_ratio > 0:
            severity = "info" if severity == "success" else severity
            messages.append("Есть единичные длинные примеры, часть хвостов будет обрезаться.")

        if p95_tokens and p95_tokens >= int(max_seq_length * 0.98):
            severity = "info" if severity == "success" else severity
            recommendations.append("Длина примеров близка к лимиту: полезно оставить 10-15% запаса на этапе prep.")

    if iters < 150:
        severity = "info" if severity == "success" else severity
        messages.append("Небольшое число итераций подходит для smoke-теста, но может быть мало для стабильного качества.")

    if not messages:
        messages.append("Параметры выглядят сбалансированными для MLX fine-tuning.")
...
def tab_generation():
    ...
    if is_mlx_gen:
        ...
        from mlx_integration import (
            ...
            load_mlx_model, mlx_generate, load_user_models,
        )
        ...
        if not MLX_AVAILABLE:
            ...
            # Build combined options: local models first, then curated + user HF models
            _local_labels = [f"📁 {d.name}" for d in _local_models]
            _all_hf_models = list(CURATED_MODELS) + load_user_models()
            _hf_labels = [get_model_display_label(m) for m in _all_hf_models]
            ...
            if _sel_idx < len(_local_models):
                ...
                mlx_gen_model_id = _all_hf_models[_sel_idx - len(_local_models)].hf_id
            ...
            # Явно показываем, что именно пойдёт в генерацию.
            st.caption(f"🎯 Effective model: `{mlx_gen_model_id}`")
            st.caption(
                f"🧩 Effective adapter: `{mlx_gen_adapter_path}`"
                if mlx_gen_adapter_path
                else "🧩 Effective adapter: `None`"
            )
            ...
            # Инициализация session state для MLX генерации
            if "mlx_gen_log" not in st.session_state:
                st.session_state.mlx_gen_log = []
            if "mlx_gen_result" not in st.session_state:
                st.session_state.mlx_gen_result = None

            btn_col, clear_col = st.columns([4, 1])
            with btn_col:
                do_generate = st.button("✨ Сгенерировать (MLX)", type="primary", width="stretch", key="mlx_gen_btn")
            with clear_col:
                if st.button("🗑️ Очистить", key="mlx_gen_clear"):
                    st.session_state.mlx_gen_log = []
                    st.session_state.mlx_gen_result = None
                    st.rerun()
            ...
            if do_generate:
                if not mlx_gen_prompt.strip():
                    ...
                    def _glog(msg: str):
                        st.session_state.mlx_gen_log.append(msg)
                        app_logger.info(f"MLX gen: {msg}")
                    ...
                    if st.session_state.get("mlx_gen_cache_key") != cache_key:
                        with st.spinner(f"Загрузка модели {mlx_gen_model_id}..."):
                            try:
                                _glog(f"Загрузка модели: {mlx_gen_model_id}")
                                if mlx_gen_adapter_path:
                                    _glog(f"Адаптер: {mlx_gen_adapter_path}")
                                ...
                                _glog("✅ Модель загружена")
                            except Exception as e:
                                _glog(f"❌ Ошибка загрузки: {e}")
                                st.error(f"Ошибка загрузки: {e}")
                                st.stop()
                    else:
                        _glog(f"♻️ Cache hit: {cache_key}")
                        _glog(f"Используем уже загруженную модель: {mlx_gen_model_id}")
                        _glog(
                            f"Адаптер: {mlx_gen_adapter_path}"
                            if mlx_gen_adapter_path
                            else "Адаптер: None"
                        )
                    ...
                    with st.spinner("Генерация..."):
                        try:
                            _glog(f"Промпт ({len(mlx_gen_prompt)} симв.), max_tokens={mlx_gen_max_tokens}, temp={mlx_gen_temperature}")
                            ...
                            template_info = "chat template" if stats.get("used_chat_template") else "raw prompt"
                            _glog(
                                f"✅ Готово: {stats['tokens_generated']} токенов, "
                                f"{stats['tokens_per_second']:.1f} tok/s, "
                                f"{stats['total_time']:.2f}s [{template_info}]"
                            )
                            st.session_state.mlx_gen_result = (mlx_gen_prompt, generated, stats)
                            ...
                            _glog(f"❌ Ошибка генерации: {e}")
                            st.error(f"Ошибка генерации: {e}")

            # --- Результат (персистентный, вне кнопки) ---
            if st.session_state.mlx_gen_result:
                _, generated, stats = st.session_state.mlx_gen_result
                st.divider()
                ...
                with sc4:
                    st.metric("Промпт токенов", stats['prompt_tokens'])
                st.text_area("Результат", value=generated, height=300, disabled=True)

            # --- Лог генерации (персистентный) ---
            if st.session_state.mlx_gen_log:
                with st.expander("📋 Лог генерации", expanded=False):
                    for entry in st.session_state.mlx_gen_log:
                        st.text(entry)
    ...
    # Инициализация session state для PyTorch генерации
    if "pt_gen_log" not in st.session_state:
        st.session_state.pt_gen_log = []
    if "pt_gen_result" not in st.session_state:
        st.session_state.pt_gen_result = None

    btn_col, clear_col = st.columns([4, 1])
    with btn_col:
        do_pt_generate = st.button("✨ Сгенерировать", type="primary", width='stretch')
    with clear_col:
        if st.button("🗑️ Очистить", key="pt_gen_clear"):
            st.session_state.pt_gen_log = []
            st.session_state.pt_gen_result = None
            st.rerun()
    ...
    if do_pt_generate:
        if not prompt.strip():
            ...
            def _ptlog(msg: str):
                st.session_state.pt_gen_log.append(msg)
                app_logger.info(f"PT gen: {msg}")

            _ptlog(f"Промпт ({len(prompt)} симв.), max_tokens={max_tokens}, temp={temperature}")
            with st.spinner("Генерация..."):
                try:
                    ...
                    _ptlog(
                        f"✅ Готово: {stats['tokens_generated']} токенов, "
                        f"{stats['tokens_per_second']:.1f} tok/s, {stats['total_time']:.2f}s"
                    )
                    st.session_state.pt_gen_result = (prompt, generated, stats)
    ...
    # --- Результат (персистентный, вне кнопки) ---
    if st.session_state.pt_gen_result:
        _, generated, stats = st.session_state.pt_gen_result
        st.divider()
    ...
    # --- Лог (персистентный) ---
    if st.session_state.pt_gen_log:
        with st.expander("📋 Лог генерации", expanded=False):
            for entry in st.session_state.pt_gen_log:
                st.text(entry)
...
def tab_training():
    ...
    # Гарантируем наличие start_time на старом кэшированном объекте
    if not hasattr(training_state, 'start_time'):
        training_state.start_time = 0.0
    ...
    with col_left:
        ...
        mlx_safety_block_reason = ""

        if is_mlx:
            st.markdown("### 🤖 MLX настройки")
            st.info("Параметры PyTorch скрыты в режиме MLX Fine-tuning.")
            from mlx_integration import (
                MLX_AVAILABLE, CURATED_MODELS, get_model_display_label,
                DataPrepConfig, prepare_mlx_dataset, load_user_models,
            )
            ...
            if not MLX_AVAILABLE:
                ...
                _all_hf_train_models = list(CURATED_MODELS) + load_user_models()
                _hf_train_labels = [get_model_display_label(m) for m in _all_hf_train_models]
                ...
                if _sel_train_idx < len(_local_train_models):
                    ...
                    selected_model = _all_hf_train_models[_sel_train_idx - len(_local_train_models)]
                ...
                mlx_dataset_goal_label = st.selectbox(
                    "Цель подготовки датасета",
                    [
                        "🤖 Авто (определить по содержимому)",
                        "🧪 Задачи (вопрос + решение + ответ)",
                        "📚 Обогащение знаниями (учебник/документация)",
                    ],
                    index=0,
                    disabled=training_state.active,
                    help=(
                        "Режим влияет на разбиение исходного текста:\n"
                        "• Авто: скрипт сам определяет задачи это или корпус\n"
                        "• Задачи: одна запись = одна цельная задача (не режется на абзацы)\n"
                        "• Обогащение: текст режется на чанки по длине для language modeling"
                    ),
                )
                if mlx_dataset_goal_label.startswith("🧪"):
                    mlx_dataset_mode = "tasks"
                elif mlx_dataset_goal_label.startswith("📚"):
                    mlx_dataset_mode = "corpus"
                else:
                    mlx_dataset_mode = "auto"
                ...
                mlx_auto_train_split = st.checkbox(
                    "🤖 Авто split train/valid",
                    value=True,
                    disabled=training_state.active,
                    help=(
                        "Автоматически подбирает split по размеру датасета.\n"
                        "Для задач <50 примеров будет 80/20."
                    ),
                )
                if mlx_auto_train_split:
                    mlx_train_split = 0.9
                else:
                    mlx_train_split = st.slider(
                        "Доля train",
                        min_value=0.5,
                        max_value=0.95,
                        value=0.9,
                    ...
                    with st.spinner("Подготовка данных..."):
                        try:
                            prep_kwargs = dict(
                                ...
                                dataset_mode=mlx_dataset_mode,
                                format=mlx_format,
                                system_prompt=mlx_system_prompt,
                                train_split=mlx_train_split,
                                auto_train_split=mlx_auto_train_split,
                                max_seq_length=mlx_max_seq_length,
                                clean_forum=mlx_clean_forum_data,
                            )
                            prep_config, dropped_params = build_dataprep_config_compat(
                                DataPrepConfig,
                                **prep_kwargs,
                            )
                            if "dataset_mode" in dropped_params:
                                st.warning(
                                    "Текущий процесс использует старую версию DataPrepConfig без dataset_mode. "
                                    "Перезапустите Streamlit, чтобы включить режимы 'Задачи/Обогащение'. "
                                    "Сейчас подготовка выполнится в базовом режиме."
                                )
                            ...
                            effective_mode = stats.get("dataset_mode_effective")
                            if effective_mode:
                                st.info(f"Режим подготовки: {effective_mode}")
                            if "full_task_train" in stats and "full_task_valid" in stats:
                                st.caption(
                                    "Цельные задачи: "
                                    f"train {stats['full_task_train']}/{stats['train_count']}, "
                                    f"valid {stats['full_task_valid']}/{stats['valid_count']}"
                                )
                            if "train_split_effective" in stats:
                                st.caption(
                                    "Использованный split: "
                                    f"{stats['train_split_effective']:.2f} "
                                    f"({stats.get('train_split_mode', 'manual')})"
                                )
                            if stats.get("deduplicated_removed", 0) > 0:
                                st.info(f"Удалено дубликатов: {stats['deduplicated_removed']}")
                            st.rerun()
                        except Exception as e:
                ...
                with st.expander("⚙️ LoRA параметры", expanded=False):
                    ...
                    with lora_col2:
                        ...
                        mlx_val_batches = st.number_input(
                            "Val batches",
                            min_value=1,
                            max_value=200,
                            value=25,
                            ...
                            disabled=training_state.active,
                            help=(
                                "Сколько батчей использовать при валидации на каждом eval.\n\n"
                                "Меньше значение = быстрее и меньше потребление памяти."
                            ),
                        )
                ...
                mlx_advice = build_mlx_training_advice(
                    source_path=mlx_source_path,
                    jsonl_dir=mlx_data_dir,
                    max_seq_length=int(mlx_max_seq_length),
                    batch_size=int(mlx_batch_size_val),
                    iters=int(mlx_iters),
                    val_batches=int(mlx_val_batches),
                    grad_checkpoint=bool(mlx_grad_checkpoint),
                )
                if mlx_advice:
                    st.divider()
                    st.caption("📊 Рекомендатор MLX")
                    metric_col1, metric_col2, metric_col3 = st.columns(3)
                    with metric_col1:
                        st.metric("Оценка токенов", f"{mlx_advice['estimated_tokens']:,}")
                    with metric_col2:
                        st.metric("Train примеров", f"{mlx_advice['train_count']:,}")
                    with metric_col3:
                        st.metric("Сверх лимита", f"{mlx_advice['over_limit']:,}")

                    advice_text = "\n".join(f"• {m}" for m in mlx_advice["messages"])
                    if mlx_advice["severity"] == "warning":
                        st.warning(advice_text)
                    elif mlx_advice["severity"] == "info":
                        st.info(advice_text)
                    elif mlx_advice["severity"] == "error":
                        st.error(advice_text)
                    else:
                        st.success(advice_text)

                    if mlx_advice["recommendations"]:
                        st.caption("Рекомендуемые действия:")
                        st.markdown("\n".join(f"- {r}" for r in mlx_advice["recommendations"]))

                mlx_mem_pressure = int(mlx_batch_size_val) * int(mlx_max_seq_length)
                if platform.system() != "Darwin" or platform.machine().lower() not in {"arm64", "aarch64"}:
                    mlx_safety_block_reason = "MLX Fine-tuning поддерживается только на Apple Silicon (arm64)."
                elif mlx_mem_pressure >= 8192:
                    mlx_safety_block_reason = (
                        "Конфигурация слишком тяжёлая для стабильной работы: "
                        f"batch_size × max_seq_length = {mlx_mem_pressure}. "
                        "Уменьшите batch size/длину контекста."
                    )

                if mlx_safety_block_reason:
                    st.error(f"⛔ {mlx_safety_block_reason}")
                elif mlx_mem_pressure >= 4096 and not mlx_grad_checkpoint:
                    st.warning(
                        "⚠️ Высокая нагрузка на unified memory. "
                        "Включите Gradient checkpointing, чтобы снизить риск OOM."
                    )
        ...
        if not is_mlx:
            st.markdown("### 🔧 PyTorch настройки")
            st.info("Параметры MLX скрыты в режимах обучения с нуля и дообучения.")
            # Параметры обучения
            st.divider()
            ...
            min_epochs_max = int(epochs) if epochs else 10
            st.session_state.setdefault("train_min_epochs", min(20, min_epochs_max))
            if st.session_state.train_min_epochs > min_epochs_max:
                st.session_state.train_min_epochs = min_epochs_max

            min_epochs = st.number_input(
                "Минимальное количество эпох",
                min_value=0,
                max_value=min_epochs_max,
                key="train_min_epochs",
                help=(
    ...
    with col_right:
        ...
        with button_col1:
            ...
            if is_mlx:
                ...
                can_start = (
                    ...
                    and not mlx_safety_block_reason
                )
        ...
        clear_log_btn = st.button(
            "🗑️ Очистить логи",
            disabled=training_state.active,
            help="Сбросить логи и метрики предыдущего запуска",
        )
        if clear_log_btn:
            training_state.logs = []
            training_state.metrics = {
                ...
                'best_val_loss': float('inf'), 'progress': 0.0,
            }
            st.rerun()
        ...
        if start_button:
            ...
            if is_mlx:
                ...
                app_logger.info(
                    f"UI MLX Параметры: model={mlx_model_id}, iters={mlx_iters}, "
                    f"batch={mlx_batch_size_val}, seq={mlx_max_seq_length}, lr={mlx_lr}, "
                    f"grad_checkpoint={mlx_grad_checkpoint}, data={mlx_data_dir}"
                )
            else:
            ...
            def run_training():
                ...
                import re as _re2
                # ⏱️  Time: 00:01:41 | Step 200: train_loss=4.0618, val_loss=5.1707
                _re_step_log = _re2.compile(
                    r'Step\s+(\d+):\s+train_loss=([\d.]+),\s+val_loss=([\d.]+)'
                )
                # ✓ Сохранён лучший checkpoint (val_loss=5.1707) → ...
                _re_best_log = _re2.compile(r'val_loss=([\d.]+).*лучший')

                # Хендлер: всё что пишется в training_logger → training_state.logs + метрики
                class _UIHandler(_logging.Handler):
                    def emit(self, record: _logging.LogRecord) -> None:
                        msg = record.getMessage()
                        training_state.logs.append(msg)
                        # Обновляем метрики из логов в реальном времени
                        m = _re_step_log.search(msg)
                        if m:
                            training_state.metrics['step'] = int(m.group(1))
                            training_state.metrics['train_loss'] = float(m.group(2))
                            training_state.metrics['val_loss'] = float(m.group(3))
                            return
                        m = _re_best_log.search(msg)
                        if m:
                            training_state.metrics['best_val_loss'] = float(m.group(1))

                _ui_handler = _UIHandler()
                _train_logger = _logging.getLogger("pytorch_llm.training")
                _train_logger.addHandler(_ui_handler)
                ...
                try:
                    if is_finetuning and checkpoint_to_load:
                        ...
                        training_state.metrics['total_steps'] = train_config.n_epochs * len(train_loader)
                    ...
                    training_state.stop_requested = False
                    _train_logger.removeHandler(_ui_handler)
        ...
        if training_state.active or training_state.logs:
            ...
            if training_state.active:
                ...
                if is_mlx:
                    ...
                    mlx_backend = training_state.metrics.get('backend')
                    mlx_on_gpu = training_state.metrics.get('using_metal_gpu')
                    ram_percent = training_state.metrics.get('ram_percent')
                    ram_used_gb = training_state.metrics.get('ram_used_gb')
                    ram_total_gb = training_state.metrics.get('ram_total_gb')
                    swap_percent = training_state.metrics.get('swap_percent')
                    swap_used_gb = training_state.metrics.get('swap_used_gb')
                    process_rss_gb = training_state.metrics.get('process_rss_gb')
                    process_pid = training_state.metrics.get('process_pid')

                    st.caption("ℹ️ MLX LoRA работает итерациями, не эпохами")
                    if mlx_backend:
                        if mlx_on_gpu is True:
                            st.caption(f"🍎 Backend: {mlx_backend} (Metal/GPU)")
                        elif mlx_on_gpu is False:
                            st.caption(f"⚠️ Backend: {mlx_backend} (CPU)")
                        else:
                            st.caption(f"ℹ️ Backend: {mlx_backend}")

                    if ram_percent is not None and swap_used_gb is not None:
                        mem_text = (
                            f"RAM {ram_used_gb:.2f}/{ram_total_gb:.2f} GB ({ram_percent:.1f}%), "
                            f"SWAP {swap_used_gb:.2f} GB ({swap_percent:.1f}%)"
                        )
                        if process_rss_gb is not None:
                            pid_suffix = f"pid={int(process_pid)}" if process_pid else "pid=n/a"
                            mem_text += f", mlx_lm RSS {process_rss_gb:.2f} GB ({pid_suffix})"

                        mem_critical = (ram_percent >= 92.0) or (swap_used_gb >= 2.0) or (swap_percent >= 25.0)
                        mem_warning = (ram_percent >= 85.0) or (swap_used_gb >= 0.5) or (swap_percent >= 5.0)

                        if mem_critical:
                            st.error(f"🚨 Memory pressure: {mem_text}")
                        elif mem_warning:
                            st.warning(f"⚠️ Memory pressure: {mem_text}")
                        else:
                            st.success(f"✅ Memory: {mem_text}")

                    m_col1, m_col2, m_col3 = st.columns(3)
                    with m_col1:
                        st.metric("Итерация", f"{mlx_step} / {mlx_iters}")
                        pct = int(mlx_step / mlx_iters * 100) if mlx_iters > 0 else 0
                        st.caption(f"{pct}% выполнено")
                    with m_col2:
                        st.metric("Train Loss", f"{mlx_train_loss:.4f}" if mlx_train_loss else "—")
                        if mlx_val_loss:
                            st.metric("Val Loss", f"{mlx_val_loss:.4f}")
                    with m_col3:
                        elapsed = (time.time() - training_state.start_time) if training_state.start_time > 0 else 0
                        st.metric("Прошло", _fmt_elapsed(elapsed))
                        if mlx_it_sec and mlx_step > 0:
                            remaining = (mlx_iters - mlx_step) / mlx_it_sec
                            st.caption(f"{mlx_it_sec:.2f} it/s • ~{_fmt_elapsed(remaining)} осталось")
                else:
                    # PyTorch progress
                    metric_col1, metric_col2, metric_col3 = st.columns(3)
                    with metric_col1:
                        st.metric("Эпоха", f"{training_state.metrics['epoch']} / {epochs}")
                        total_steps = training_state.metrics.get('total_steps', 0)
                        step_label = f"{training_state.metrics['step']} / {total_steps}" if total_steps else str(training_state.metrics['step'])
                        st.metric("Шаг", step_label)
                    with metric_col2:
                    ...
                    with metric_col3:
                        elapsed = (time.time() - training_state.start_time) if training_state.start_time > 0 else 0
                        st.metric("Прошло", _fmt_elapsed(elapsed))
                        pt_progress = training_state.metrics.get('progress', 0.0)
                        if pt_progress > 0 and elapsed > 0:
                            remaining_pt = elapsed * (1 - pt_progress) / pt_progress
                            st.caption(f"~{_fmt_elapsed(remaining_pt)} осталось")
            ...
            if is_mlx and not training_state.active and training_state.metrics.get('finished'):
                ...
                fuse_dequantize = st.checkbox(
                    "Fuse с dequantize (больше размер, потенциально лучше качество)",
                    value=False,
                    key="mlx_fuse_dequantize",
                    help=(
                        "Отключено по умолчанию: fused модель остается квантизованной и компактной.\n\n"
                        "Включайте только если хотите dequantized fused-модель и готовы к заметному росту размера."
                    ),
                )
                if st.button(
                    ...
                    with st.spinner("Слияние адаптера..."):
                        ...
                        success, msg = fuse_adapter(
                            mlx_model_id,
                            mlx_adapter_path,
                            fuse_output,
                            dequantize=fuse_dequantize,
                        )
                        if success:
                            st.success(f"✅ {msg}")
                        else:
            ...
            # Логи
            st.divider()
            log_count = len(training_state.logs)
            st.caption(f"**Логи** ({log_count} строк):")
            logs_container = st.container(height=400)
            with logs_container:
                import re as _re
                _re_mlx_iter = _re.compile(
                    r"Iter\s+(\d+):\s+Train loss\s+([\d.]+),\s+Learning Rate\s+([\S]+),\s+It/sec\s+([\d.]+)"
                    r"(?:,\s+Tokens/sec\s+([\d.]+))?(?:.*?Peak mem\s+([\S]+) GB)?"
                )
                _re_val = _re.compile(r"Val loss\s+([\d.]+)")
                for log in reversed(training_state.logs):
                    if not log.strip() or set(log.strip()) <= {"─", "-", "="}:
                        st.divider()
                        continue
                    m_iter = _re_mlx_iter.search(log)
                    m_val = _re_val.search(log)
                    if m_iter:
                        # MLX сырая строка — форматируем
                        step = m_iter.group(1)
                        loss = m_iter.group(2)
                        lr = m_iter.group(3)
                        it_s = m_iter.group(4)
                        tok_s = m_iter.group(5) or "—"
                        peak = m_iter.group(6) or "—"
                        st.markdown(
                            f"**Iter {step}** — loss `{loss}` | "
                            f"lr `{lr}` | {it_s} it/s | {tok_s} tok/s | mem {peak} GB"
                        )
                    elif m_val and not any(c.isalpha() and ord(c) > 127 for c in log[:5]):
                        # MLX Val loss строка (не PyTorch с кириллицей)
                        st.markdown(f"📊 **Val loss** `{m_val.group(1)}`")
                    else:
                        # Всё остальное — PyTorch логи с emoji, markdown, plain text
                        st.markdown(log)
  ...
  --num-layers {mlx_lora_layers} \\
  --batch-size {mlx_batch_size_val} \\
  --iters {mlx_iters} \\
  --max-seq-length {mlx_max_seq_length} \\
  --val-batches {mlx_val_batches} \\
  --steps-per-eval {mlx_steps_per_eval} \\
  --save-every {mlx_save_every}"""

        with st.expander("Показать команду для MLX Fine-tuning"):
            st.code(command, language="bash")
            st.caption("Скопируйте и запустите в терминале для MLX LoRA обучения без UI")

    elif is_finetuning and checkpoint_to_load:
...
def tab_info():
    ...
    # Управление пользовательским списком моделей
    st.subheader("➕ Мои модели HF")
    from mlx_integration import load_user_models, save_user_model, remove_user_model, is_model_cached

    _user_models = load_user_models()
    _add_col1, _add_col2 = st.columns([3, 1])
    with _add_col1:
        _new_hf_id = st.text_input(
            "HuggingFace ID модели",
            placeholder="mlx-community/Mistral-7B-Instruct-v0.3-4bit",
            key="new_user_model_id",
            label_visibility="collapsed",
        )
    with _add_col2:
        if st.button("➕ Добавить", key="add_user_model", disabled=not _new_hf_id):
            save_user_model(_new_hf_id.strip())
            st.success(f"Добавлено: {_new_hf_id.strip()}")
            st.rerun()

    if _user_models:
        for _um in _user_models:
            _uc1, _uc2 = st.columns([4, 1])
            with _uc1:
                _cached_mark = "✅" if is_model_cached(_um.hf_id) else "⬇️"
                st.text(f"{_cached_mark} {_um.hf_id}")
            with _uc2:
                if st.button("🗑️", key=f"del_user_{_um.hf_id}"):
                    remove_user_model(_um.hf_id)
                    st.rerun()
    else:
        st.caption("Пока нет сохранённых моделей.")
...
def tab_logs():
    ...
    with col4:
        st.caption("")  # label spacer
        if st.button("🗑️ Очистить", key="clear_log_file", help="Очистить содержимое выбранного лог-файла"):
            log_path_to_clear = logs_dir / selected_log
            log_path_to_clear.write_text("", encoding="utf-8")
            st.rerun()
